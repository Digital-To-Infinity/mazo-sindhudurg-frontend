...
const PublishingSettings = ({ 
    ...
    return (
        <div className="ag-card p-5 max-[426px]:p-4 space-y-4 shadow-sm border-slate-100/50 bg-white rounded-xl">
            <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest pl-1 border-b border-slate-100 pb-2">Publishing Settings</h4>

            <div className="grid grid-cols-1 gap-6">
                <div className="space-y-4">
                    <div className="relative">
                        <label className="text-xs font-semibold text-slate-500 flex items-center gap-2 mb-2">
                            <LayoutGrid className="w-4 h-4" /> Category
                        </label>
                        <select
                            value={category}
                            onChange={(e) => setCategory(e.target.value)}
                            className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all appearance-none"
                        >
                ...
                <div className="space-y-4">
                    <div className="flex flex-col space-y-2">
                        <label className="text-xs font-semibold text-slate-500">Post Status</label>
                        <div className="flex items-center space-x-2 bg-slate-50 p-1.5 rounded-xl border border-slate-100">
                            <button
                                onClick={() => setStatus('Published')}
                                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all cursor-pointer ${status === 'Published' ? 'bg-white text-emerald-600 shadow-sm border border-emerald-50' : 'text-slate-500 hover:text-black'}`}
                            >
                                {status === 'Published' ? 'Published' : 'Publish'}
                            </button>
                            <button
                                onClick={() => setStatus('Draft')}
                                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all cursor-pointer ${status === 'Draft' ? 'bg-white text-amber-600 border border-amber-50' : 'text-slate-500 hover:text-black'}`}
                            >
                                Draft
                            </button>
                        </div>
